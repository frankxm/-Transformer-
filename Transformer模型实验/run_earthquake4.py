...
if __name__ == '__main__':
    ...
    BATCH_SIZE=64
    optimizer_list=['Adam','Adagrad','RMSprop','Nadam','SGD','SGD with momentum=0.9']
    ...
    for op in optimizer_list:
        ...
        ax3.plot(loss_list)
        ax3.set_xlabel(f'epoch*{batch_times}')
        ax3.set_ylabel('train_loss')
        ax3.set_title(f'loss in train')
        plt.gca()
        ax3.legend(labels=['Adam', 'Adagrad', 'RMSprop', 'Nadam', 'SGD', 'SGD with momentum=0.9'], loc='upper right')

        ax4.plot(correct_on_train)
        ax4.set_xlabel(f'epoch')
        ax4.set_ylabel('correct')
        ax4.set_title('CORRECT on Train')
        plt.gca()
        ax4.legend(labels=['Adam', 'Adagrad', 'RMSprop', 'Nadam', 'SGD', 'SGD with momentum=0.9'],loc='lower right')

        ax5.plot(val_loss)
        ax5.set_xlabel(f'epoch*{batch_val_times}')
        ax5.set_ylabel('val_loss')
        ax5.set_title(f'mean loss in test')
        plt.gca()
        ax5.legend(labels=['Adam', 'Adagrad', 'RMSprop', 'Nadam', 'SGD', 'SGD with momentum=0.9'], loc='lower right')

        ax6.plot(correct_on_test)
        ax6.set_xlabel(f'epoch')
        ax6.set_ylabel('correct')
        ax6.set_title('CORRECT on Test')
        plt.gca()
        ax6.legend(labels=['Adam', 'Adagrad', 'RMSprop', 'Nadam', 'SGD', 'SGD with momentum=0.9'],loc='lower right')

        count+=1

    plt.show()
    for i in range(len(optimizer_list)):
        print(f'optimizer为{optimizer_list[i]}:')
        print(f'batchsize为{BATCH_SIZE}:',end='')
